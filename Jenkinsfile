        ...
        steps {
            script {
                sshagent(['ec2-ssh']) {
                    ...
                    sh "CONTAINER_NAME="emp-mgt-be-container"; if docker ps -q --filter name="$CONTAINER_NAME" | grep -q .; then docker stop "$CONTAINER_NAME"; else echo "Container '$CONTAINER_NAME' is not running."; fi"
